...
// ----------------------
$filtrosSQL = ["f.batalhao IN ($idsPermitidosStr)"]; // Restringe à OM do usuário e subordinadas
$params = [];
$tipos = '';
...
$stmtFrota->bind_param($tiposFrota, ...$paramsFrota);
$stmtFrota->execute();
$resViaturas = $stmtFrota->get_result();

$viaturas = [];
while ($vtr = $resViaturas->fetch_assoc()) $viaturas[] = $vtr;
$stmtFrota->close();

// ----------------------
// Calcular manutenção preventiva
// ----------------------
...
foreach ($viaturas as $v) {
    ...
    if ($filtrosGET['status_manut'] && strtolower($manut['status']) !== strtolower($filtrosGET['status_manut'])) continue;
    $v['manut'] = $manut;
    $viaturasFiltradas[] = $v;
}
...
// ----------------------
// ---------------------------------------------------------
// PAGINAÇÃO FICHAS (similar à paginação da frota)
// ---------------------------------------------------------
