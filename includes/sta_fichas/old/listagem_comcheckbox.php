...
foreach ($campos as $coluna => $parametro) {
    if (!empty($_GET[$parametro])) {
        ...
        if (str_contains($coluna, '>=')) {
            ...
            $params[] = $valor;
            $tipos .= 's';
        } elseif ($parametro === 'id' || $parametro === 'batalhao') {
            $filtros[] = "$coluna = ?";
            $params[] = (int)$valor;
            $tipos .= 'i';
        } else {
...
// CONSULTA TOTAL
// ============================
$sqlTotal = "SELECT COUNT(*) as total FROM sta_fichas $condicoes";
...
// PAGINAÇÃO
// ============================
function renderPaginacaoFichas($pagina, $totalPaginas, $limite, $queryString, $arquivo = 'includes/fichas/listagem.php') {
    ...
    if ($totalPaginas > 1) {
        for ($i = 1; $i <= $totalPaginas; $i++) {
            ...
            $html .= "<li class='page-item $ativo'>
                        <a class='page-link paginacao-fichas' href='#' data-page='{$url}'>$i</a>
                      </li>";
        }
    }
    $html .= '</ul></nav></div>';
 ...
 <div class="paginacao">
  ...
  <?php while ($row = $result->fetch_assoc()): ?>
    <?php
      ...
      if ($id_viatura) {
          $stmtValor = $conexao->prepare("SELECT prefixo_sga FROM frota WHERE id = ?");
          $stmtValor->bind_param("i", $id_viatura);
          $stmtValor->execute();
          $res = $stmtValor->get_result();
          if ($res->num_rows > 0) {
              $dados = $res->fetch_assoc();
              $prefixo_sga = $dados['prefixo_sga'] ?? '—';
          }
          $stmtValor->close();
      }
    ?>
...
?>
  ...
  <select class="form-select" name="id_viatura" required>
          ...
          <div class="form-check form-check-inline">
            ...
            <label class="form-check-label" for="manutencaoPreventiva">A vtr/eqp retornou?</label>
          </div>
