...
$nivel_usuario = $resNivel->fetch_assoc()['nivel'] ?? 0;
$stmtNivel->close();
...
if ($nivel_usuario == 1) {
    ...
    while ($r = $res->fetch_assoc()) $batalhoesPermitidos[] = $r['id'];
} elseif ($nivel_usuario == 2) {
    // Nível 2 vê o próprio e os subordinados
    ...
    $stmt->execute();
    $res = $stmt->get_result();
    while ($r = $res->fetch_assoc()) $batalhoesPermitidos[] = $r['id_om_menor'];
    $stmt->close();
} else {
...
if (!empty($batalhaoFiltro) && in_array((int)$batalhaoFiltro, $batalhoesPermitidos)) {
    ...
    // Garante que só veja os batalhões permitidos
...
// TOTAL DE REGISTROS
// ============================
$sqlTotal = "SELECT COUNT(*) as total FROM fin_pedidos_forn p $condicoes";
...
$sql = "
  SELECT 
    p.*, 
    f.prefixo_sga,
    om.nome AS nome_batalhao, om.abreviatura AS abreviatura_batalhao
    ...
    <div class="list-group">
      <?php if ($pedidos->num_rows > 0): ?>
        <?php while ($pedido = $pedidos->fetch_assoc()): ?>
          <?php
          $stmt = $conexao->prepare("SELECT COUNT(*) AS total_itens FROM fin_pedidos_forn_itens WHERE id_principal = ?");
          ...
          <div class="list-group-item list-group-item-action flex-column align-items-start mb-3 p-3 border rounded-3 shadow-sm">
            <div class="d-flex w-100 justify-content-between align-items-center">
              <h6 class="mb-0 fw-semibold text-primary">Pedido #<?= htmlspecialchars($pedido['id']) ?> - <?= htmlspecialchars($pedido['abreviatura_batalhao'] ?? 'Não informado') ?></h6>
              
            </div>

           <p class="mb-2 text-secondary">Prefixo/Vtr: <?= htmlspecialchars($pedido['prefixo_sga'] ?? 'Não informado') ?></p>

            <div class="d-flex flex-column small text-body-secondary">
              <div><strong>Local do pedido:</strong> <?= htmlspecialchars($pedido['local_pedido']) ?></div>
              <div><strong>Solicitante:</strong> <?= htmlspecialchars($pedido['solicitante']) ?></div>
              <div><strong>Data do Pedido:</strong> <?= htmlspecialchars($pedido['data_pedido']) ?></div>
  ...
  <ul class="dropdown-menu dropdown-menu-end shadow-sm rounded" aria-labelledby="dropdownMenu<?= $pedido['id'] ?>">
   ...
   data-id="<?= $pedido['id'] ?>">
    <i class="fas fa-file-pdf me-2"></i> Imprimir PEDIDO
                ...
                <button class="btn btn-sm btn-outline-secondary" type="button" data-bs-toggle="collapse" data-bs-target="#itensPedido<?= $pedido['id'] ?>" aria-expanded="false" aria-controls="itensPedido<?= $pedido['id'] ?>">
...
>
  <i class="fas fa-edit me-1"></i> Editar
              ...
              <button 
  type="button"
  class="btn btn-sm btn-outline-danger d-flex align-items-center gap-1 btn-deletar-pedido"
  data-id="<?= $pedido['id'] ?>"
...
>
  ...
  <span class="d-none d-md-inline">Remover</span>
            ...
            <div class="collapse mt-3" id="itensPedido<?= $pedido['id'] ?>">
                  ...
                  <ul class="list-group list-group-flush">
                    <?php while($item = $resItens->fetch_assoc()): ?>
                      <li class="list-group-item">
                        <strong>Item:</strong> <?= htmlspecialchars($item['descricao_item'] ?? 'N/A') ?>
                        — <strong>Qtd:</strong> <?= htmlspecialchars($item['quant_solicitada'] ?? 0) ?>
                        — <strong>Valor:</strong> R$ <?= number_format($item['valor_unt'], 2, ',', '.') ?>
                      </li>
                    <?php endwhile; ?>
                  </ul>
                <?php else: ?>
