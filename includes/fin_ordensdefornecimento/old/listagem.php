...
$campos = [
    'o.id'                     => ['param' => 'id', 'tipo' => 'i'],
    'o.nome_responsavel'       => ['param' => 'nome_responsavel', 'tipo' => 's', 'like' => true],
    'o.data_cadastro >=' => ['param' => 'data_ini', 'tipo' => 's'],
    'o.data_cadastro <=' => ['param' => 'data_fim', 'tipo' => 's']
];

// ==================================================
// 🔹 Filtro padrão: últimos 90 dias (se não informar datas)
// ==================================================
if (empty($_GET['data_ini']) && empty($_GET['data_fim'])) {
    $_GET['data_ini'] = date('Y-m-d', strtotime('-90 days'));
...
// ==================================================
foreach ($campos as $coluna => $config) {
    $param = $config['param'];
...
// ==================================================
// 🔹 Filtro direto por batalhão (se enviado)
// ==================================================
if (!empty($_GET['batalhao']) && is_numeric($_GET['batalhao'])) {
    $filtros[] = "o.batalhao = ?";
    $params[]  = (int) $_GET['batalhao'];
    $tipos    .= 'i';
}
...
function renderPaginacao($pagina, $totalPaginas, $limite, $queryString, $arquivo = 'includes/fin_ordensdefornecimento/listagem.php') {
    ...
    if ($pagina > 1) {
        $html .= "<li class='page-item'><a class='page-link paginacao' href='#' data-page='{$makeUrl(1)}'>&laquo; Primeira</a></li>";
        $html .= "<li class='page-item'><a class='page-link paginacao' href='#' data-page='{$makeUrl($pagina - 1)}'>&lsaquo;</a></li>";
    }
    ...
    for ($i = $inicio; $i <= $fim; $i++) {
        ...
        $html .= "<li class='page-item {$active}'><a class='page-link paginacao' href='#' data-page='{$makeUrl($i)}'>{$i}</a></li>";
    }
    ...
    if ($pagina < $totalPaginas) {
        $html .= "<li class='page-item'><a class='page-link paginacao' href='#' data-page='{$makeUrl($pagina + 1)}'>&rsaquo;</a></li>";
        $html .= "<li class='page-item'><a class='page-link paginacao' href='#' data-page='{$makeUrl($totalPaginas)}'>Última &raquo;</a></li>";
    }
  ...
  <div class="page-inner">
    <div class="d-flex justify-content-between align-items-center py-3">
      <div>
        ...
        <h6 class="text-muted">Para melhorar o desempenho do sistema, são exibidos inicialmente apenas as ordens de fornecimento dos últimos 90 dias.
    ...
    <div class="list-group">
      ...
      <?php if ($ordens->num_rows > 0): ?>
        <?php while ($ordem = $ordens->fetch_assoc()): ?>
          ...
          <?php
            // Empenho (já vem leve do backend ou exibe padrão)
            $nmr_empenho = !empty($ordem['nmr_empenho'] ?? null)
              ? $ordem['nmr_empenho']
              : 'Empenho não vinculado';

            // Total da ordem (opcional — pode vir null)
            $total_ordemforn = (float) ($ordem['total_ordem'] ?? 0);
          ?>
          ...
          <div class="list-group-item list-group-item-action flex-column mb-4 p-4 border rounded-3 shadow-sm">
            ...
            <div class="d-flex justify-content-between align-items-start">
              <div class="d-flex flex-column small text-body-secondary">
                ...
                <h6 class="fw-bold text-primary mb-2">
                  Ordem de Fornecimento #<?= $ordem['id'] ?> - <?= $ordem['abreviatura_om'] ?>
                </h6>
                ...
                <div>
                  ...
                  <?= !empty($ordem['data_cadastro']) 
                        ? (new DateTime($ordem['data_cadastro']))->format('d/m/Y') 
                        : '—' ?>
                </div>

                <div>
                  <strong>Data Limite de Entrega:</strong>
                  <?= !empty($ordem['data_entrega_limite']) 
                        ? (new DateTime($ordem['data_entrega_limite']))->format('d/m/Y') 
                        : '—' ?>
                </div>
                ...
                <div>
                  ...
                  <?= htmlspecialchars($ordem['nome_responsavel']) ?> —
                  <?= htmlspecialchars($ordem['contato_responsavel']) ?>
                </div>

                <div>
                  <strong>Valor Total da Ordem:</strong>
                  R$ <?= number_format($total_ordemforn, 2, ',', '.') ?>
                </div>

              </div>

              <span class="badge rounded-pill bg-<?= 
                $ordem['status'] === 'Agd entrega' ? 'warning' :
                ($ordem['status'] === 'Concluída' ? 'success' : 'secondary') ?> mt-3">
              ...
              <div class="d-flex ms-auto align-items-center gap-2 mt-3 flex-wrap">
                ...
                <div class="dropdown">
                  ...
                  <ul class="dropdown-menu dropdown-menu-end shadow-sm rounded">
                    <li>
                      <a href="#" class="dropdown-item text-danger btnExportarPDFordem"
                         data-id="<?= $ordem['id'] ?>">
                        <i class="fas fa-file-pdf me-2"></i> Imprimir OF
                      </a>
                    </li>
                    <li>
                      <a href="#" class="dropdown-item text-danger btnExportarPDFofpedidos"
                         data-id="<?= $ordem['id'] ?>">
                ...
                <!-- Lazy loading -->
                <button
                  class="btn btn-outline-secondary btn-sm btn-ver-pedidos"
                  data-id="<?= $ordem['id'] ?>"
                  data-bs-toggle="collapse"
                  data-bs-target="#pedidosOrdem<?= $ordem['id'] ?>">
                ...
                <button type="button"
                        class="btn btn-sm btn-outline-danger btn-deletar-of"
                        data-id="<?= $ordem['id'] ?>"
            ...
            <!-- 🔽 CONTAINER DO LAZY LOADING -->
            <div class="collapse mt-3 pedidos-container"
                 id="pedidosOrdem<?= $ordem['id'] ?>"
                 data-loaded="0">
              <div class="text-muted small">
                <b>Carregando</b> os dados da ordem de fornecimento…
              </div>
            </div>
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form method="POST" id="form-ordem-cadastrar">
          ...
          <?php
// ==== Batalhões Permitidos (mesma lógica do Local da OS) ====
$batalhoesPermitidos = [];
...
if ($nivel_usuario == 1) {
    ...
    while ($row = $res->fetch_assoc()) {
        $batalhoesPermitidos[] = (int)$row['id'];
    }
} else {
    $batalhoesPermitidos[] = (int)$id_om_usuario;
    ...
    while ($r = $resSubs->fetch_assoc()) {
        $batalhoesPermitidos[] = (int)$r['id_om_menor'];
    }
    $stmtSubs->close();
}
...
if (!empty($batalhoesPermitidos)) {
    ...
    $sqlEmpenhos = "
        SELECT e.id, e.nmr_empenho, om.nome AS nome_batalhao
        FROM fin_empenhos e
        JOIN fin_requisicao r ON r.id = e.id_requisicao
        JOIN organizacoes_militares om ON om.id = r.batalhao
        WHERE r.batalhao IN ($placeholders)
        ORDER BY e.id DESC
    ";

    $stmtEmp = $conexao->prepare($sqlEmpenhos);
    $tipos = str_repeat('i', count($batalhoesPermitidos));

    // mesmo padrão do seu local_os (bind_param com spread)
    $stmtEmp->bind_param($tipos, ...$batalhoesPermitidos);
...
<div class="col-md-12">
  ...
  <select class="form-select rounded-pill shadow-sm" id="id_empenho" name="id_empenho" required>
    <option value="" disabled selected>Selecione o empenho</option>

    <?php foreach ($empenhos as $e): ?>
      <option value="<?= (int)$e['id'] ?>">
        <?= htmlspecialchars($e['nmr_empenho'] . ' - ' . $e['nome_batalhao']) ?>
      </option>
    <?php endforeach; ?>

    <?php if (empty($empenhos)): ?>
      <option value="" disabled>Nenhum empenho disponível</option>
    <?php endif; ?>
  </select>
</div>
...
<div class="modal fade" id="modalImportarOrdens" tabindex="-1">
  <div class="modal-dialog">
    <div class="modal-content">
      <form id="formImportarOrdem" method="POST" enctype="multipart/form-data">
        <div class="modal-header">
          <h5 class="modal-title">Importar Planilha de Ordens / Pedidos</h5>
...
<?php
          // ============================
          // BATALHÕES QUE O USUÁRIO PODE VER
          // ============================
          $id_om_usuario = $_SESSION['usuario']['batalhao'] ?? 0;
          ...
          if ($nivelUsuario == 1) {
              ...
              // Usuário comum – vê OM dele + OMs subordinadas
              $sqlBatalhoes = "
                  SELECT om.id, om.nome, om.abreviatura
                  FROM organizacoes_militares om
                  WHERE om.id = $id_om_usuario
...
?>
          ...
          <!-- SELECIONAR BATALHÃO (VEM DO FORMULÁRIO, NÃO DO EXCEL) -->
          <label class="form-label">Selecione o Batalhão</label>
          <select name="batalhao" id="batalhao_import_ordens" class="form-select mb-3" required>
            <option value="">Selecione...</option>
            <?php while ($bat = $resBatalhoes->fetch_assoc()): ?>
              <option value="<?= $bat['id'] ?>">
                <?= htmlspecialchars($bat['abreviatura'] . ' - ' . $bat['nome']) ?>
          ...
          <input type="file" name="arquivo" id="arquivo_ordens" 
                 accept=".xlsx" class="form-control mb-3" required>

          <!-- LINK PARA BAIXAR MODELO -->
          <a href="includes/fin_ordensdefornecimento/planilha_modelo_ordens.xlsx" class="btn btn-link p-0">
...
document.querySelectorAll('.btn-ver-pedidos').forEach(btn => {
  btn.addEventListener('click', function () {
    ...
    const container = document.querySelector('#pedidosOrdem' + ordemId);

    if (container.dataset.loaded === "1") return;

    fetch('includes/fin_ordensdefornecimento/carregar_pedidos.php?id_ordem=' + ordemId)
      .then(r => r.text())
      .then(html => {
      ...
      .catch(() => {
        container.innerHTML = '<div class="alert alert-danger">Erro ao carregar pedidos.</div>';
      });
  });
});
</script>