...
function renderPaginacao($pagina, $totalPaginas, $limite, $queryString, $arquivo = 'includes/fin_ordensdefornecimento/listagem.php') {
    ...
    if ($pagina > 1) {
        $html .= "<li class='page-item'>
                    <a class='page-link paginacao' href='#' data-page='" . $makeUrl(1) . "'>&laquo; Primeira</a>
                  </li>";
    }
    ...
    if ($pagina > 1) {
        $prev = $pagina - 1;
        $html .= "<li class='page-item'>
                    <a class='page-link paginacao' href='#' data-page='" . $makeUrl($prev) . "'>&lsaquo;</a>
                  </li>";
    }
    ...
    for ($i = $inicio; $i <= $fim; $i++) {
        ...
        $html .= "<li class='page-item {$ativo}'>
                    <a class='page-link paginacao' href='#' data-page='" . $makeUrl($i) . "'>$i</a>
                  </li>";
    }
    ...
    if ($pagina < $totalPaginas) {
        $next = $pagina + 1;
        $html .= "<li class='page-item'>
                    <a class='page-link paginacao' href='#' data-page='" . $makeUrl($next) . "'>&rsaquo;</a>
                  </li>";
    }
    ...
    if ($pagina < $totalPaginas) {
        $html .= "<li class='page-item'>
                    <a class='page-link paginacao' href='#' data-page='" . $makeUrl($totalPaginas) . "'>Última &raquo;</a>
                  </li>";
    }
...
$stmtValorTotal = $conexao->prepare("
    SELECT 
        SUM(
            itens.total_itens - (itens.total_itens * (p.desconto_empenho / 100))
        ) AS total
    FROM fin_ordemforn_pedidos op
    JOIN fin_pedidos_forn p ON p.id = op.id_pedido
    JOIN (
        SELECT id_principal, SUM(valor_total) AS total_itens
        FROM fin_pedidos_forn_itens
        GROUP BY id_principal
    ) itens ON itens.id_principal = p.id
    WHERE op.id_ordemforn = ?
");
